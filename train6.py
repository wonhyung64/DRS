...
from module.metric import ndcg_func_ssm, recall_func_ssm, ap_func_ssm
...
parser.add_argument("--balance-param", type=float, default=0.5)
...
wandb.run.name = f"ours6_{expt_num}"
...
for epoch in range(1, num_epochs+1):
    ...
    for idx in range(total_batch):
        ...
        _, user_embed, item_embed = model(org_x)
        ...
        if contrast_pair == "user":
            user_sim_loss = contrastive_loss(user_embed, pos_user_embed, temperature) * balance_param
            cl_loss = user_sim_loss 
            epoch_user_sim_loss += user_sim_loss
            epoch_cl_loss += cl_loss
            
        elif contrast_pair == "item":
            item_sim_loss = contrastive_loss(item_embed, pos_item_embed, temperature) * balance_param
    ...
    if epoch % evaluate_interval == 0:
        ...
        ndcg_res = ndcg_func_ssm(model, x_test, y_test, device, top_k_list)
        ndcg_dict: dict = {}
        for top_k in top_k_list:
            ndcg_dict[f"ndcg_{top_k}"] = np.mean(ndcg_res[f"ndcg_{top_k}"])
        wandb_var.log(ndcg_dict)

        recall_res = recall_func_ssm(model, x_test, y_test, device, top_k_list)
        recall_dict: dict = {}
        for top_k in top_k_list:
            recall_dict[f"recall_{top_k}"] = np.mean(recall_res[f"recall_{top_k}"])
        wandb_var.log(recall_dict)

        ap_res = ap_func_ssm(model, x_test, y_test, device, top_k_list)
        ap_dict: dict = {}
        for top_k in top_k_list:
