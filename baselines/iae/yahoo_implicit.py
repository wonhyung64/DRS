...
from model import IAE
from loss import l2_loss, squred_loss
from metric import iae_ndcg_func
...
wandb.run.name = f"iae_{expt_num}"
...
sparse_train = sparse_train.transpose()

train_dict = {}
for idx, value in enumerate(sparse_train):
    train_dict[idx] = value.indices.copy().tolist()

total_batch = num_items // batch_size

#%% TRAIN
model = IAE(num_users, num_items)
...
for epoch in range(1, num_epochs+1):
    ...
    for idx in range(total_batch):
        ...
        sub_x = np.zeros((batch_size, num_users))
        for i, item_id in enumerate(selected_idx):
            users_by_item_id = train_dict[item_id]
            sub_x[i, users_by_item_id] = 1
    ...
    if epoch % evaluate_interval == 0:
        ...
        ndcg_res = iae_ndcg_func(model, x_test, y_test, train_dict, device, top_k_list)
        ndcg_dict: dict = {}
