...
from module.loss import angle_contrastive_loss
from module.similarity import corr_sim, cosine_sim
...
for epoch in range(1, num_epochs+1):
    ...
    for idx in range(total_batch):
        ...
        user_x, user_y = torch.meshgrid(sub_pos_user, sub_pos_user)
        user_sim = user_user_sim[user_x.flatten(), user_y.flatten()].reshape([batch_size, batch_size]).to(device)

        item_x, item_y = torch.meshgrid(sub_pos_item, sub_pos_item)
        item_sim = item_item_sim[item_x.flatten(), item_y.flatten()].reshape([batch_size, batch_size]).to(device)

        if contrast_pair == "user":
            user_sim_loss = angle_contrastive_loss(anchor_user_embed, pos_user_embed, user_sim, temperature) * balance_param
            cl_loss = user_sim_loss 
            epoch_user_sim_loss += user_sim_loss
            epoch_cl_loss += cl_loss
            
        elif contrast_pair == "item":
            item_sim_loss = angle_contrastive_loss(anchor_item_embed, pos_item_embed, item_sim, temperature) * balance_param
