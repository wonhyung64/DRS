...
theta_loc_list = [0., 30., 50.] #normal 10^4
# theta_loc_list = [0., 0.32, 1.5] #uniform 10^4
...
for theta_loc in theta_loc_list:
    ...
    for repeat_seed in tqdm(range(1, repeat_num+1)):
        ...
        y1 = random_df[random_df['T'] == 1].reset_index(drop=True)["Y"].to_numpy()
        y0 = random_df[random_df['T'] == 0].reset_index(drop=True)["Y"].to_numpy()
        ...
        y1 = real_df[real_df['T'] == 1].reset_index(drop=True)["Y"].to_numpy()
        q1 = real_df[real_df['T'] == 1].reset_index(drop=True)["propensity"].to_numpy()
        y0 = real_df[real_df['T'] == 0].reset_index(drop=True)["Y"].to_numpy()
        q0 = real_df[real_df['T'] == 0].reset_index(drop=True)["propensity"].to_numpy()

        ##### real mean estimation
        real_mean = y1.mean() - y0.mean()
        real_mean_list.append(random_mean)
        ...
        ##### real ipw estimation #####
        ipw_mean = (y1 / q1).sum()/N - (y0 / (1 - q0)).sum()/N
        ipw_mean_list.append(ipw_mean)

        ##### real data ipw estimation #####
        # propensity = q * t + (1-q)*(1-t)
        # ipw_model = LogisticRegression(C=10000)
        # ipw_model.fit(real_xt, y, sample_weight=1/propensity)
    ...
    #     [[(random_coef_arr[:,i] - beta_XT_to_Y[i+1]).mean().round(4), random_coef_arr[:,i].var().round(4)] for i in range(1+d)]
    # }\n""")
    ...
    #     [[(real_coef_arr[:,i] - beta_XT_to_Y[i+1]).mean().round(4), real_coef_arr[:,i].var().round(4)] for i in range(1+d)]
    # }\n""")
    ...
    #     [[(ipw_coef_arr[:,i] - beta_XT_to_Y[i+1]).mean().round(4), ipw_coef_arr[:,i].var().round(4)] for i in range(1+d)]
    # }\n""")
