...
if __name__ == "__main__":
	...
	e_feat = torch.rand(N, embedding_size)
	target = torch.randint(high=C, size=(N,))
	v_proxy = torch.rand(C, embedding_size)
	
	# soft_plus = nn.Softplus()
	label = torch.LongTensor([i for i in range(C)])
	
	e_norm = F.normalize(e_feat, p=2, dim=1)
	v_norm = F.normalize(v_proxy, p=2, dim=1)

	pred = F.linear(e_norm, v_norm)  # (N, C)

	target_label = (label.unsqueeze(1).to(e_norm.device) == target.unsqueeze(0))  # (C, N)
	
	pred = torch.masked_select(pred.transpose(1, 0), target_label)  # N,
	...
	logits = torch.cat([pred, feature], dim=1)  # (N, 1+N)
	label = torch.zeros(logits.size(0), dtype=torch.long).to(feature.device)
	loss = F.nll_loss(F.log_softmax(scale * logits, dim=1), label)
